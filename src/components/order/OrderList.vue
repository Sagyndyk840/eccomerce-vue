...
export default {
  ...
  setup () {
    ...
    return { v$: useVuelidate(), cartStore }
  },
  data () {
    return {
      // changeAddress: '',
      // changePayment: '',
      order: {
        fio: JSON.parse(sessionStorage.getItem('user')).name ?? '',
  ...
  methods: {
    async send () {
      ...
      await this.cartStore.createCart(this.order)
    }
  },
  validations () {
    return {
...
<!--            </h2>-->
            ...
            <div class="order-left__personal"> <!-- <div class="order-left__personal" v-if="changeAddress === 'address'"> -->
              <Input v-model:value="order.address"
                     :errors=" v$.order.address.$errors"
                     type="text"
          ...
          <div class="order-right">
            ...
            <div class="order-right__inner">
              ...
              <div class="order-right__item">
                ...
                <div class="order-right__item--strong">{{ cartStore.cart.amount }} грн</div>
              </div>
            </div>

            <Button type="submit" :loader="cartStore.loading" class-name="bg-yellow color-white btn-cart" title="ОФОРМИТЬ ЗАКАЗ">
